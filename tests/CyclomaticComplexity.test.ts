...
describe("CyclomaticComplexity ", () => {
  const example_uri = path.join(__dirname, "../files/example-flows/force-app/main/default/flows/Cyclomatic_Complexity.flow-meta.xml");
  const other_uri = path.join(__dirname, "../files/example-flows/force-app/main/default/flows/SOQL_Query_In_A_Loop.flow-meta.xml");
  const defaultConfig = {
    rules: {
