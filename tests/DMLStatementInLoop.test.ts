...
describe("DMLStatementInLoop ", () => {
  const example_uri = path.join(__dirname, "../assets/example-flows/force-app/main/default/flows/DML_Statement_In_A_Loop.flow-meta.xml");
  const fixed_uri = path.join(__dirname, "../assets/example-flows/force-app/main/default/flows/Duplicate_DML_Operation_Fixed.flow-meta.xml");
  const config = {
    rules: {
      DMLStatementInLoop: {
        severity: "error",
